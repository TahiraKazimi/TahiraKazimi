...
bool Flight::addTicket(const Ticket &ticket) {
    /* TODO */
    int ticket_size= tickets.size(); bool isFull;
    if(ticket.getTicketType()==economy) isFull = ticket_size<economyCapacity;
    else isFull = ticket_size<businessCapacity;
    if(isFull&& ticket.getTicketType()==economy){ 
        tickets.push_back(ticket);
    }
    else if(isFull && ticket.getTicketType()==business){
        tickets.push_back(ticket);
    }
    return isFull;
}
...
bool Flight::operator<(const Flight &rhs) const {
    /* TODO */
    if(this->flightCode < rhs.flightCode) return true;
    else return false;
}
